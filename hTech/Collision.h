...
namespace Collision
{
	...
	/// </summary>
	class HTECH_FUNCTION_EXPORT GJK
	{
	private:
		...
		Vector2 EPA(Collider* bodyA, Collider* bodyB);
		...
		void GetCollisionDetails(RigidbodyComponent* bodyA, RigidbodyComponent* bodyB, CollisionManifold* manifold);
		...
		/// <param name="bodyB"></param>
		/// <returns></returns>
		bool DetermineCollisionUsingSimplex(Collider* bodyA, Collider* bodyB);
		...
		/// <param name="bodyB"></param>
		/// <returns></returns>
		Vector2 GetMinkowskiDifferenceSupportVertex(const Vector2& direction, Collider* bodyA, Collider* bodyB);
	...
	public:
		...
		/// <param name="manifold"></param>
		/// <returns></returns>
		bool IsOverlapping(RigidbodyComponent* bodyA, RigidbodyComponent* bodyB, CollisionManifold* manifold);
	};

	static bool CheckCollision(const RigidbodyComponent& one, const RigidbodyComponent& two, CollisionManifold* manifold = nullptr)
	{
		Collision::GJK gjk;
		return gjk.IsOverlapping(const_cast<RigidbodyComponent*>(&one), const_cast<RigidbodyComponent*>(&two), manifold);
	}
};