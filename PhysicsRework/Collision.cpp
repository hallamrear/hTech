...
Vector2 Collision::GJK::GetMinkowskiDifferenceSupportVertex(const Vector2& direction, Body* bodyA, Body* bodyB)
{
	Vector2 directionNorm = direction.GetNormalized();
	Vector2 supportA = bodyA->GetSupportVertex(directionNorm);
	Vector2 oppositeDir = directionNorm * -1;
	Vector2 supportB = bodyB->GetSupportVertex(oppositeDir);
...
Vector2 Collision::GJK::GetSupportVertexOfSimplex(const Vector2& direction)
{
	...
	for (size_t i = 0; i < m_SimplexPoints.size(); i++)
	{
		...
		projectedDistance = MathsUtils::Dot(vertex, directionNorm);
...
bool Collision::GJK::IsOverlapping(Body* bodyA, Body* bodyB, Manifold* manifold)
{
	m_SimplexPoints.clear();

	bool isOverlapping = DetermineCollisionUsingSimplex(bodyA, bodyB);
...
{
	return (MathsUtils::Dot(A, B) > 0);
}

Vector2 Collision::GJK::EPA(Body* bodyA, Body* bodyB)
{
	Vector2 MTV = Vector2();
...
void Collision::GJK::GetCollisionDetails(Body* bodyA, Body* bodyB, Manifold* manifold)
{
	Vector2 MTV = EPA(bodyA, bodyB);
...
bool Collision::GJK::DetermineCollisionUsingSimplex(Body* bodyA, Body* bodyB)
{
	//Set out initial search direciton in the direciton of the second body.
	Vector2 searchDirection = Vector2(bodyB->Pos - bodyA->Pos);
	...
	while (true)
	{
		...
		if (MathsUtils::Dot(supportVertex, searchDirection) <= 0.0f)
		{
			return false;
		}
...
bool Collision::GJK::CalculateSimplexPointFromTriangle(Vector2& searchDirection)
{
	...
	Vector2 abf = MathsUtils::TripleProduct(ac, ab, ab).GetNormalized();
	Vector2 acf = MathsUtils::TripleProduct(ab, ac, ac).GetNormalized();
...
bool Collision::GJK::CalculateSimplexPointFromLine(Vector2& searchDirection)
{
	...
	if (AreVectorsInSameDirection(ab, ao))
	{
		...
		searchDirection = MathsUtils::TripleProduct(ab, ao, ab).GetNormalized();
	}
	else
	{
		m_SimplexPoints.erase(m_SimplexPoints.begin() + 1);
